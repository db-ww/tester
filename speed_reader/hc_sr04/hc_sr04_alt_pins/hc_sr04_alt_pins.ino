/*
 * HC-SR04 Ultrasonic Distance Sensor - ESP32 (Alternative Pins)
 * 
 * Connections:
 * VCC -> 5V (or 3.3V)
 * GND -> GND
 ...
 * ECHO -> GPIO 14
 * 
 * This version uses different GPIO pins in case GPIO 5/18 have conflicts
 * 
...
// Pin definitions (ESP32 GPIO pins - Alternative)
const int TRIG_PIN = 12;  // GPIO 12
const int ECHO_PIN = 14;  // GPIO 14
...
void setup() {
  ...
  Serial.println("ESP32 Debug Version (Alt Pins)");
  Serial.println("====================================");
...
void loop() {
  ...
  if (duration == 0) {
    ...
    Serial.println("     - TRIG connected to GPIO 12?");
    Serial.println("     - ECHO connected to GPIO 14?");
